...
def create_deck():
    ...
    # Theme Color Palette - High Contrast Cybersecurity & FinTech
    BG_DARK = RGBColor(7, 10, 19)          # #070a13
    CARD_BG = RGBColor(15, 23, 42)         # #0f172a
    CARD_BORDER = RGBColor(30, 41, 59)     # #1e293b
    CARD_BORDER_ACTIVE = RGBColor(30, 58, 138) # #1e3a8a
    ...
    # Left Hero Container
    add_card(s1, 0.8, 0.8, 7.3, 5.9, CARD_BG, CARD_BORDER_ACTIVE)
    
    # Event Badge
    badge = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.2), Inches(1.2), Inches(5.6), Inches(0.42))
    badge.fill.solid()
    ...
    # Brand Title
    tb = s1.shapes.add_textbox(Inches(1.2), Inches(1.8), Inches(6.5), Inches(1.5))
    tf = tb.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    p.text = "SURAKSHA AI"
    ...
    p2 = tf.add_paragraph()
    p2.text = "Real-Time Payment Risk & Fraud Intelligence"
    p2.font.size = Pt(17)
    p2.font.bold = True
    p2.font.color.rgb = CYAN_ACCENT

    # Slogan / Pitch Hook
    tb_val = s1.shapes.add_textbox(Inches(1.2), Inches(3.5), Inches(6.5), Inches(1.5))
    ...
    if os.path.exists(logo_path):
        s1.shapes.add_picture(logo_path, Inches(8.55), Inches(1.1), width=Inches(3.8))
        
    caption_box = s1.shapes.add_textbox(Inches(8.55), Inches(5.7), Inches(3.8), Inches(0.7))
    tf_c = caption_box.text_frame
    tf_c.word_wrap = True
    p = tf_c.paragraphs[0]
    p.text = "Prototype validated across real-world payment attack vectors"
    p.font.size = Pt(10.5)
    p.font.bold = True
    ...
    problems = [
        ("01", "QR Sticker Swapping", "Physical Retail Tampering", 
         ...
         "Silent fund diversion at checkout", RED_ALERT),
        ("02", "Doctored Payment Screenshots", "Visual Image Forgery", 
        ...
        ("03", "Phishing & Urgency Traps", "Social Engineering Traps", 
         ...
         "Victims realize only after bank debit", RED_ALERT)
    ]

    for i, (num, title, subtitle, desc, stat, color) in enumerate(problems):
        x = 0.8 + i * 4.0
        add_card(s2, x, 1.55, 3.7, 5.15, CARD_BG, CARD_BORDER)
        ...
        num_box = s2.shapes.add_textbox(Inches(x + 0.25), Inches(1.8), Inches(1.0), Inches(0.55))
        ...
        c_box = s2.shapes.add_textbox(Inches(x + 0.25), Inches(2.45), Inches(3.2), Inches(4.0))
        tf = c_box.text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]
        p.text = title
    ...
    add_card(s3, 0.8, 1.55, 5.7, 5.15, CARD_BG, RGBColor(75, 85, 99))
    add_card(s3, 6.8, 1.55, 5.7, 5.15, CARD_BG, RGBColor(30, 58, 138))
    ...
    p = tf_l.paragraphs[0]
    p.text = "❌ Current Digital Payment Experience"
    p.font.size = Pt(17)
    p.font.bold = True
    ...
    p = tf_r.paragraphs[0]
    p.text = "🛡️ Suraksha AI: Pre-Authorization Shield"
    p.font.size = Pt(17)
    p.font.bold = True
    p.font.color.rgb = GREEN_SAFE
    ...
    apply_bg(s4)
    add_header(s4, "THE SOLUTION", "Meet Suraksha AI: Multi-Modal Risk Intelligence", CYAN_ACCENT)
    ...
    for i, (title, img_name, sub) in enumerate(demo_cards):
        x = 0.8 + i * 4.0
        add_card(s5, x, 1.55, 3.7, 5.15, CARD_BG, CARD_BORDER)
        ...
        tb_sub = s5.shapes.add_textbox(Inches(x + 0.2), Inches(5.95), Inches(3.3), Inches(0.6))
    ...
    for i, (title, bullets, col) in enumerate(tech_cols):
        x = 0.8 + i * 4.0
        add_card(s6, x, 1.55, 3.7, 5.15, CARD_BG, CARD_BORDER)
    ...
    add_card(s7, 0.8, 1.55, 11.7, 5.15, CARD_BG, CARD_BORDER)
    ...
    for i, (title, subtitle, bullets, col) in enumerate(biz_pillars):
        x = 0.8 + i * 4.0
        add_card(s8, x, 1.55, 3.7, 5.15, CARD_BG, CARD_BORDER)
