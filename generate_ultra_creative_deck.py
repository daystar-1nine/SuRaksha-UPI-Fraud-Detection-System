...
def create_ultra_deck():
    ...
    CARD_HIGHLIGHT = RGBColor(30, 58, 138)  # #1e3a8a Active Blue Glow
    ...
    def set_slide_bg(slide, bg_name):
        bg_path = os.path.join(bg_dir, f"{bg_name}_bg.png")
        if os.path.exists(bg_path):
            slide.shapes.add_picture(bg_path, 0, 0, width=Inches(13.333), height=Inches(7.5))

    def add_glass_card(slide, left, top, width, height, bg_col=CARD_BG, border_col=CARD_BORDER, border_w=1.2):
    ...
    def add_header(slide, tag_text, title_text, category_color=CYAN_ACCENT):
        # Header Badge
        tag_box = slide.shapes.add_textbox(Inches(0.8), Inches(0.35), Inches(10.5), Inches(0.35))
        ...
        if os.path.exists(logo_file):
            try:
                slide.shapes.add_picture(logo_file, Inches(11.8), Inches(0.38), height=Inches(0.7))
            except Exception:
                pass

    # ==========================================
    # SLIDE 1: THE HOOK / COMMAND CENTER
    # ==========================================
    s1 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s1, "slide1")
    
    # Left Hero Glass Card with Cyan Glow
    add_glass_card(s1, 0.8, 0.8, 7.3, 5.9, CARD_BG, CYAN_ACCENT, 1.8)
    ...
    add_glass_card(s1, 8.4, 0.8, 4.1, 5.9, CARD_BG, CARD_BORDER)
    ...
    # ==========================================
    # SLIDE 2: THE PROBLEM (CRIME SCENE)
    # ==========================================
    s2 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s2, "slide2")
    add_header(s2, "THE DIGITAL PAYMENT CRISIS", "Instant Payments. Delayed Fraud Awareness.", RED_ALERT)

    problems = [
        ("01", "Physical QR Sticker Swapping", "Point-of-Sale Tampering", 
         "Fraudsters paste deceptive scam QR stickers over legitimate shop counter stands. Funds are silently diverted to attacker accounts at checkout.",
         "Threat: Silent checkout fund diversion", RED_ALERT),
        ("02", "Doctored Payment Screenshots", "Digital Visual Image Forgery", 
         "Customers flash fabricated GPay / PhonePe receipt screenshots made via photo editing apps. Merchants hand over goods without receiving funds.",
         "Threat: Merchants lose daily working capital", AMBER_WARN),
        ("03", "Phishing & Urgency Traps", "Social Engineering Links", 
         "Scammers send WhatsApp / SMS links disguised as 'Cashback' or 'KYC Update'. Clicking triggers an unauthorized debit request (`upi://pay`).",
         "Threat: Victims realize only after bank debit", RED_ALERT)
    ]

    for i, (num, title, subtitle, desc, stat, color) in enumerate(problems):
        x = 0.8 + i * 4.0
        add_glass_card(s2, x, 1.55, 3.7, 5.25, CARD_BG, color, 1.5)
    ...
    s2.notes_slide.notes_text_frame.text = (
        "[TIME: 0:30 - 1:10]\n"
        "[WHAT TO SAY]: Digital payments are fast and seamless, but fraud tactics have evolved to exploit the human interface before bank settlement. Today's payment scams target users through three distinct vectors: physical QR sticker swapping, doctored confirmation screenshots, and social engineering phishing links. The core problem is that users lack real-time risk intelligence at the exact moment of payment.\n"
        "[ACTION]: Point to each of the 3 problem cards on the screen.\n"
        "[TRANSITION]: Why does this vulnerability exist in our current apps? That brings us to our core insight."
    )

    # ==========================================
    # SLIDE 3: THE CORE INSIGHT (PARADIGM SHIFT)
    # ==========================================
    s3 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s3, "slide3")
    add_header(s3, "THE CORE INSIGHT", "The Security Gap Exists Before The Payment", BLUE_ACCENT)

    # Left: Traditional
    add_glass_card(s3, 0.8, 1.55, 5.7, 5.25, CARD_BG, RED_ALERT, 1.5)
    # Right: Suraksha
    add_glass_card(s3, 6.8, 1.55, 5.7, 5.25, CARD_BG, GREEN_SAFE, 1.8)
    ...
    p = tf_l.paragraphs[0]
    p.text = "[TRADITIONAL] Current Payment Experience"
    p.font.size = Pt(16.5)
    p.font.bold = True
    ...
    points_l = [
        ...
        ("Blind Trust in QR URLs: ", "Apps parse raw links without validating physical counter sticker integrity."),
        ("Zero Receipt Defense: ", "Merchants have no built-in tools to detect Photoshop-edited receipts."),
        ("Language Barrier: ", "Technical security jargon confuses non-English speaking retail shopkeepers.")
    ]
    ...
    p = tf_r.paragraphs[0]
    p.text = "[PRE-AUTH SHIELD] Suraksha AI Zero-Trust"
    p.font.size = Pt(16.5)
    p.font.bold = True
    p.font.color.rgb = GREEN_SAFE
    ...
    # ==========================================
    # SLIDE 4: THE SOLUTION (4-PILLAR DEFENSE GRID)
    # ==========================================
    s4 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s4, "slide4")
    add_header(s4, "THE SOLUTION", "Meet Suraksha AI: Multi-Modal Defense Grid", CYAN_ACCENT)
    ...
    for i, (title, desc, col) in enumerate(pillars):
        x = 0.8 + i * 2.95
        add_glass_card(s4, x, 1.55, 2.75, 2.35, CARD_BG, col, 1.5)
    ...
    add_glass_card(s4, 0.8, 4.15, 11.7, 2.65, RGBColor(15, 23, 42), CYAN_ACCENT, 1.8)
    ...
    # ==========================================
    # SLIDE 5: PRODUCT EVIDENCE (SOC MISSION CONTROL)
    # ==========================================
    s5 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s5, "slide5")
    ...
    for i, (title, img_name, sub) in enumerate(demo_cards):
        x = 0.8 + i * 4.0
        add_glass_card(s5, x, 1.55, 3.7, 5.25, CARD_BG, GREEN_SAFE if i==1 else CYAN_ACCENT, 1.5)
    ...
    # ==========================================
    # SLIDE 6: TECHNICAL ARCHITECTURE (BLUEPRINT)
    # ==========================================
    s6 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s6, "slide6")
    ...
    for i, (title, bullets, col) in enumerate(tech_cols):
        x = 0.8 + i * 4.0
        add_glass_card(s6, x, 1.55, 3.7, 5.25, CARD_BG, col, 1.5)
    ...
    # ==========================================
    # SLIDE 7: DIFFERENTIATION (BATTLECARD)
    # ==========================================
    s7 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s7, "slide7")
    add_header(s7, "COMPETITIVE DIFFERENTIATION", "From Payment Execution to Payment Risk Intelligence", CYAN_ACCENT)

    add_glass_card(s7, 0.8, 1.55, 11.7, 5.25, CARD_BG, CYAN_ACCENT, 1.5)
    ...
    # ==========================================
    # SLIDE 8: BUSINESS MODEL (SCALE FUNNEL)
    # ==========================================
    s8 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s8, "slide8")
    ...
    for i, (title, subtitle, bullets, col) in enumerate(biz_pillars):
        x = 0.8 + i * 4.0
        add_glass_card(s8, x, 1.55, 3.7, 5.25, CARD_BG, col, 1.5)
    ...
    # ==========================================
    # SLIDE 9: THE CLOSE (THE GRAND OATH)
    # ==========================================
    s9 = prs.slides.add_slide(blank_layout)
    set_slide_bg(s9, "slide9")

    add_glass_card(s9, 1.2, 0.95, 10.9, 5.6, CARD_BG, CYAN_ACCENT, 2.0)
    ...
    for p_out in out_paths:
        try:
            prs.save(p_out)
            print(f"Saved ultra-creative presentation to: {p_out}")
